# Roteiro de conclusão — planos de ação dos painéis no Iris

> Para seguir no Claude Code. Estado em 08/09/2026: **código publicado nos quatro apps**
> (Iris @65, Spravato v4.74, Carteira PF v8.47, GT Onco v1.39). O que falta depende do
...
Abra o projeto Apps Script do Iris (script `1z0an94Jvpt8sYf9kfWO5tMhIQ9ubLmhcL1i6B9o5YxcIdV2392xy64yG`)
...
⚠️ **A partir da gravação, as 40 pessoas passam a entrar no Iris** (a allowlist é a aba
...
1. **Iris** (`…/exec`): a aba Projetos do modal "Gerenciar Projetos" mostra os três com o chip
   `público`. O board abre filtrado nas suas tarefas; "Limpar filtros" mostra as importadas.
2. **Iris** `…/exec?projeto=<ID do GT Onco>`: abre Tarefas filtradas no projeto.
3. **Iris** `…/exec?tarefa=<ID de uma tarefa importada>`: abre o modal em visualização.
4. **Painel GT Onco**, seção Plano de Ação: KPIs, cards, botão "Abrir no Iris". Antes da
...
6. Peça a alguém **fora da aba Usuários** (ou use uma conta que não esteja nela) para abrir
   um painel: o plano deve aparecer mesmo assim. Abrir o Iris com essa conta deve dar
   "Acesso restrito".
7. **Taiara** ou **Carina** abre o Iris: entra como Gestor e vê todas as tarefas.
...
- **Comunicar a equipe** que o Iris está aberto para os 40 (fica com você).
